...
//            LogUtils.logBle("This is not an iBeacon advertisment (no 4c000215 seen in bytes 2-5).  The bytes I see are: "+bytesToHex(scanData));
            return null;
        }
     ...
     * 计算距离
     */
    public static float calculateAccuracy(int txPower, double rssi) {
        if (rssi == 0) {
            return -1.0f;
        }
...
//        return (double) (10 ^ ((Math.abs(rssi) - txPower) / (10 * 2.0)));
    }
